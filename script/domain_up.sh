...
if [ ! -f "$file" ]; then
    echo Domain $(yellow "${domain}${TLD}") on $(yellow "${platform^^}") platform $(red "not found!")
    exit 1
fi

file=${DIR_PLATFORM}/sites-enabled/${domain}${TLD}.conf
if [ -f "$file" ]; then
    echo Domain $(yellow "${domain}${TLD}") on $(yellow "${platform^^}") platform $(green "is already up and running!")
    exit 1
fi

# Process confirmation
echo Domain $(green "${domain}${TLD}") access through $(yellow "${platform^^}") platform $(yellow "will be put up online.")
prompt_confirm_proceed "Are you sure to proceed?" || exit 1

# Up on Apache if domain belongs to
